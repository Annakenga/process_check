(() => {
  ...
  const reasonsToParagraph = (reasons) =>
    reasons
      .map((reason) => {
    ...
    return `
      <div class="screen-content result-content result-${kind}">
        ...
        <div class="result-statement">
          Процесс <strong>«${escapeHtml(state.processName)}»</strong> ${content.statement}
        </div>
        ...
        ${content.listTitle ? `
          <div class="result-section">
            ...
            ${kind === "not-ready" ? `
              <p class="reason-paragraph">${reasonsToParagraph(content.reasons)}</p>
            ` : `
